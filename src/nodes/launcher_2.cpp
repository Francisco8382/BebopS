...
namespace bebop_simulator {
    ...
    class Launcher{
        ...
        private:
            double Kp_xy_i, Kp_z_i, Kp_phi_theta_i, Kp_psi_i;
            double Td_xy_i, Td_z_i, Td_phi_theta_i, Td_psi_i;
            double Ti_xy_i, Ti_z_i, Ti_phi_theta_i, Ti_psi_i;
            double Kp_xy_f, Kp_z_f, Kp_phi_theta_f, Kp_psi_f;
            double Td_xy_f, Td_z_f, Td_phi_theta_f, Td_psi_f;
            double Ti_xy_f, Ti_z_f, Ti_phi_theta_f, Ti_psi_f;
            double Kp_xy_d, Kp_z_d, Kp_phi_theta_d, Kp_psi_d;
            double Td_xy_d, Td_z_d, Td_phi_theta_d, Td_psi_d;
            double Ti_xy_d, Ti_z_d, Ti_phi_theta_d, Ti_psi_d;
            double Kp_xy, Kp_z, Kp_phi_theta, Kp_psi;
            double Td_xy, Td_z, Td_phi_theta, Td_psi;
            double Ti_xy, Ti_z, Ti_phi_theta, Ti_psi;
    ...
    void Launcher::InitializeParams() {
      ...
      bool verbose;
      ros::param::get("~Kp_xy/Kp_xy_i", Kp_xy_i);
      ros::param::get("~Kp_z/Kp_z_i", Kp_z_i);
      ros::param::get("~Kp_phi_theta/Kp_phi_theta_i", Kp_phi_theta_i);
      ros::param::get("~Kp_psi/Kp_psi_i", Kp_psi_i);
      ros::param::get("~Td_xy/Td_xy_i", Td_xy_i);
      ros::param::get("~Td_z/Td_z_i", Td_z_i);
      ros::param::get("~Td_phi_theta/Td_phi_theta_i", Td_phi_theta_i);
      ros::param::get("~Td_psi/Td_psi_i", Td_psi_i);
      ros::param::get("~Ti_xy/Ti_xy_i", Ti_xy_i);
      ros::param::get("~Ti_z/Ti_z_i", Ti_z_i);
      ros::param::get("~Ti_phi_theta/Ti_phi_theta_i", Ti_phi_theta_i);
      ros::param::get("~Ti_psi/Ti_psi_i", Ti_psi_i);
      ros::param::get("~Kp_xy/Kp_xy_f", Kp_xy_f);
      ros::param::get("~Kp_z/Kp_z_f", Kp_z_f);
      ros::param::get("~Kp_phi_theta/Kp_phi_theta_f", Kp_phi_theta_f);
      ros::param::get("~Kp_psi/Kp_psi_f", Kp_psi_f);
      ros::param::get("~Td_xy/Td_xy_f", Td_xy_f);
      ros::param::get("~Td_z/Td_z_f", Td_z_f);
      ros::param::get("~Td_phi_theta/Td_phi_theta_f", Td_phi_theta_f);
      ros::param::get("~Td_psi/Td_psi_f", Td_psi_f);
      ros::param::get("~Ti_xy/Ti_xy_f", Ti_xy_f);
      ros::param::get("~Ti_z/Ti_z_f", Ti_z_f);
      ros::param::get("~Ti_phi_theta/Ti_phi_theta_f", Ti_phi_theta_f);
      ros::param::get("~Ti_psi/Ti_psi_f", Ti_psi_f);
      ros::param::get("~Kp_xy/Kp_xy_d", Kp_xy_d);
      ros::param::get("~Kp_z/Kp_z_d", Kp_z_d);
      ros::param::get("~Kp_phi_theta/Kp_phi_theta_d", Kp_phi_theta_d);
      ros::param::get("~Kp_psi/Kp_psi_d", Kp_psi_d);
      ros::param::get("~Td_xy/Td_xy_d", Td_xy_d);
      ros::param::get("~Td_z/Td_z_d", Td_z_d);
      ros::param::get("~Td_phi_theta/Td_phi_theta_d", Td_phi_theta_d);
      ros::param::get("~Td_psi/Td_psi_d", Td_psi_d);
      ros::param::get("~Ti_xy/Ti_xy_d", Ti_xy_d);
      ros::param::get("~Ti_z/Ti_z_d", Ti_z_d);
      ros::param::get("~Ti_phi_theta/Ti_phi_theta_d", Ti_phi_theta_d);
      ros::param::get("~Ti_psi/Ti_psi_d", Ti_psi_d);
      ...
      pos_controller_node = "rosrun bebop_simulator pos_controller_node_2 _U_xyz/U_x:=" 
      ...
      plot = "rosrun bebop_simulator plot_2.py _Ruta:=" + Ruta
      ...
      Kp_xy = Kp_xy_i;
      Kp_z = Kp_z_i;
      Kp_phi_theta = Kp_phi_theta_i;
      Kp_psi = Kp_psi_i;
      Td_xy = Td_xy_i;
      Td_z = Td_z_i;
      Td_phi_theta = Td_phi_theta_i;
      Td_psi = Td_psi_i;
      Ti_xy = Ti_xy_i;
      Ti_z = Ti_z_i;
      Ti_phi_theta = Ti_phi_theta_i;
      Ti_psi = Ti_psi_i;
    ...
    void Launcher::TimerCallback(const ros::TimerEvent& event) {
      RunNodes();
      Kp_xy += Kp_xy_d;
      if (Kp_xy < Kp_xy_f && Kp_xy_d > 0){
        return;
      }
      Kp_xy = Kp_xy_i;
      Kp_z += Kp_z_d;
      if (Kp_z < Kp_z_f && Kp_z_d > 0){
        return;
      }
      Kp_z = Kp_z_i;
      Kp_phi_theta += Kp_phi_theta_d;
      if (Kp_phi_theta < Kp_phi_theta_f && Kp_phi_theta_d > 0){
        return;
      }
      Kp_phi_theta = Kp_phi_theta_i;
      Kp_psi += Kp_psi_d;
      if (Kp_psi < Kp_psi_f && Kp_psi_d > 0){
        return;
      }
      Kp_psi = Kp_psi_i;
      Td_xy += Td_xy_d;
      if (Td_xy < Td_xy_f && Td_xy_d > 0){
        return;
      }
      Td_xy = Td_xy_i;
      Td_z += Td_z_d;
      if (Td_z < Td_z_f && Td_z_d > 0){
        return;
      }
      Td_z = Td_z_i;
      Td_phi_theta += Td_phi_theta_d;
      if (Td_phi_theta < Td_phi_theta_f && Td_phi_theta_d > 0){
        return;
      }
      Td_phi_theta = Td_phi_theta_i;
      Td_psi += Td_psi_d;
      if (Td_psi < Td_psi_f && Td_psi_d > 0){
        return;
      }
      Td_psi = Td_psi_i;
      Ti_xy += Ti_xy_d;
      if (Ti_xy < Ti_xy_f && Ti_xy_d > 0){
        return;
      }
      Ti_xy = Ti_xy_i;
      Ti_z += Ti_z_d;
      if (Ti_z < Ti_z_f && Ti_z_d > 0){
        return;
      }
      Ti_z = Ti_z_i;
      Ti_phi_theta += Ti_phi_theta_d;
      if (Ti_phi_theta < Ti_phi_theta_f && Ti_phi_theta_d > 0){
        return;
      }
      Ti_phi_theta = Ti_phi_theta_i;
      Ti_psi += Ti_psi_d;
      if (Ti_psi < Ti_psi_f && Ti_psi_d > 0){
        return;
      }
      else {
        ros::shutdown();
    ...
    void Launcher::RunNodes() {
      std::string s_Kp_xy, s_Kp_z, s_Kp_phi_theta, s_Kp_psi,
      s_Td_xy, s_Td_z, s_Td_phi_theta, s_Td_psi,
      s_Ti_xy, s_Ti_z, s_Ti_phi_theta, s_Ti_psi, s_controller_params;
      std::string run_pos_controller_node, run_csv1, run_csv2, run_csv3, run_csv4,
      run_plot, run_waypoint, run_control;
      s_Kp_xy = std::to_string(Kp_xy);
      s_Kp_z = std::to_string(Kp_z);
      s_Kp_phi_theta = std::to_string(Kp_phi_theta);
      s_Kp_psi = std::to_string(Kp_psi);
      s_Td_xy = std::to_string(Td_xy);
      s_Td_z = std::to_string(Td_z);
      s_Td_phi_theta = std::to_string(Td_phi_theta);
      s_Td_psi = std::to_string(Td_psi);
      s_Ti_xy = std::to_string(Ti_xy);
      s_Ti_z = std::to_string(Ti_z);
      s_Ti_phi_theta = std::to_string(Ti_phi_theta);
      s_Ti_psi = std::to_string(Ti_psi);
      
      s_controller_params = " _Kp_xy/Kp_xy:=" + s_Kp_xy
      + " _Kp_z/Kp_z:=" + s_Kp_z
      + " _Kp_phi_theta/Kp_phi_theta:=" + s_Kp_phi_theta + " _Kp_psi/Kp_psi:=" + s_Kp_psi
      + " _Td_xy/Td_xy:=" + s_Td_xy + " _Td_z/Td_z:=" + s_Td_z 
      + " _Td_phi_theta/Td_phi_theta:=" + s_Td_phi_theta + " _Td_psi/Td_psi:=" + s_Td_psi
      + " _Ti_xy/Ti_xy:=" + s_Ti_xy + " _Ti_z/Ti_z:=" + s_Ti_z 
      + " _Ti_phi_theta/Ti_phi_theta:=" + s_Ti_phi_theta + " _Ti_psi/Ti_psi:=" + s_Ti_psi;
