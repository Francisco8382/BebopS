...
#include "bebop_simulator/pos_controller_2.h"
...
void PositionController::CallbackSaveData(const ros::TimerEvent& event){
      ...
      fileControllerGains << Kp_xy_ << "," << Kp_z_ << "," << Kp_phi_theta_ << "," << Kp_psi_ << "," << Td_xy_ << ","
    		  << Td_z_ << "," << Td_phi_theta_ << "," << Td_psi_ << odometry_.timeStampSec << "," << odometry_.timeStampNsec << "\n";
...
void PositionController::SetControllerGains(){
      ...
      Kp_xy_ = controller_parameters_.Kp_xy_;
      Kp_z_ = controller_parameters_.Kp_z_;

      Kp_phi_theta_ = controller_parameters_.Kp_phi_theta_;
      Kp_psi_ = controller_parameters_.Kp_psi_;

      Td_xy_ = controller_parameters_.Td_xy_;
      Td_z_ = controller_parameters_.Td_z_;

      Td_phi_theta_ = controller_parameters_.Td_phi_theta_;
      Td_psi_ = controller_parameters_.Td_psi_; 

      Ti_xy_ = controller_parameters_.Ti_xy_;
      Ti_z_ = controller_parameters_.Ti_z_;

      Ti_phi_theta_ = controller_parameters_.Ti_phi_theta_;
      Ti_psi_ = controller_parameters_.Ti_psi_; 
...
void PositionController::GetUTerrComponents(nav_msgs::Odometry* uTerrComponents){
  ...
  uTerrComponents->pose.pose.position.x = Kp_z_/Td_z_ * dot_e_z_;
  uTerrComponents->pose.pose.position.y = Kp_z_ * e_z_;
  uTerrComponents->pose.pose.position.z = g_ + Kp_z_ * (dot_e_z_/Td_z_ + e_z_);
...
void PositionController::VelocityErrors(double* dot_e_x, double* dot_e_y, double* dot_e_z){
   ...
   if(EKF_active_){
	   *dot_e_x = command_trajectory_.velocity_W[0] - state_.linearVelocity.x;
	   *dot_e_y = command_trajectory_.velocity_W[1] - state_.linearVelocity.y;
	   *dot_e_z = command_trajectory_.velocity_W[2] - state_.linearVelocity.z;
   }
   else{
	   ...
	   *dot_e_x = command_trajectory_.velocity_W[0] - dot_x;
   	 *dot_e_y = command_trajectory_.velocity_W[1] - dot_y;
   	 *dot_e_z = command_trajectory_.velocity_W[2] - dot_z;
...
void PositionController::PosController(double* u_T, double* phi_r, double* theta_r, double* u_x, double* u_y, double* u_z, double* u_Terr){
   ...
   *u_x = clamp(command_trajectory_.acceleration_W[0]+Kp_xy_*(e_x_ + dot_e_x_*Td_xy_ + sum_e_x_/Ti_xy_),m_*(lambda_x_-std::abs(command_trajectory_.acceleration_W[0])));
   *u_y = clamp(command_trajectory_.acceleration_W[1]+Kp_xy_*(e_y_ + dot_e_y_*Td_xy_ + sum_e_y_/Ti_xy_),m_*(lambda_y_-std::abs(command_trajectory_.acceleration_W[1])));
   *u_z = clamp(command_trajectory_.acceleration_W[2]+Kp_z_ *(e_z_ + dot_e_z_*Td_z_  + sum_e_z_/Ti_z_ ),m_*(lambda_z_-std::abs(command_trajectory_.acceleration_W[2])));
...
void PositionController::AttitudeController(double* u_phi, double* u_theta, double* u_psi){
   ...
   *u_phi = Ix_ *   ( Kp_phi_theta_ * ( ( sum_e_phi_   / Ti_phi_theta_ ) + ( dot_e_phi_   * Td_phi_theta_ ) + e_phi_   ) - ( ( (Iy_ - Iz_)/(Ix_ * mu_theta_ * mu_psi_) ) * e_theta_ * e_psi_) );
   *u_theta = Iy_ * ( Kp_phi_theta_ * ( ( sum_e_theta_ / Ti_phi_theta_ ) + ( dot_e_theta_ * Td_phi_theta_ ) + e_theta_ ) - ( ( (Iz_ - Ix_)/(Iy_ * mu_phi_ * mu_psi_  ) ) * e_phi_   * e_psi_) );
   *u_psi = Iz_ *   ( Kp_psi_       * ( ( sum_e_psi_   / Ti_psi_       ) + ( dot_e_psi_   * Td_psi_       ) + e_psi_   ) - ( ( (Ix_ - Iy_)/(Iz_ * mu_theta_ * mu_phi_) ) * e_theta_ * e_phi_) );
...
void PositionController::CallbackAttitude(const ros::TimerEvent& event){
     ...
     sum_e_phi_   += e_phi_   * TsA;
     sum_e_theta_ += e_theta_ * TsA;
     sum_e_psi_   += e_psi_   * TsA;
